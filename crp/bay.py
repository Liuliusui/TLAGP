...
from crp.common import irange, T_STEP, T_PICKUP, TRUCK_POS
...
class Bay:
    def __init__(self, n_stacks, n_tiers, conf):
        ...
        self.crane_pos = 0  # current crane column (stack index)
        self.crane_time = 0.0
    ...
    def move_crane_and_pick(self, src: int, dst: int):
        """Accumulate time for a relocation from src to dst, and update crane position."""
        self.crane_time += T_STEP * abs(self.crane_pos - src)  # go to src
        self.crane_time += T_STEP * abs(src - dst)  # move to dst
        self.crane_time += T_PICKUP  # pick/place
        self.crane_pos = dst

    def retrieve_from(self, s: int):
        """Accumulate time for retrieving target container from stack s to truck."""
        self.crane_time += T_STEP * abs(self.crane_pos - s)  # go to s
        self.crane_time += T_STEP * abs(s - TRUCK_POS)  # go to truck
        self.crane_time += T_PICKUP  # pick/place
        self.crane_pos = TRUCK_POS
